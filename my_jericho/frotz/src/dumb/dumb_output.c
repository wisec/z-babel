...
static char jericho_screen_buffer[SCREEN_BUFF_SIZE];
char* jericho_screen_buffer_ptr = jericho_screen_buffer;

void jericho_set_main_window_text(zchar c){
	if(get_current_window()== 0 ){
	 if ((jericho_screen_buffer_ptr - jericho_screen_buffer) < (SCREEN_BUFF_SIZE - 1)) {
		    //printf("%c",c);
    		*jericho_screen_buffer_ptr++ = c;
  		}
	}
}
char *jericho_get_main_window_text(void){
  *jericho_screen_buffer_ptr = '\0';
  return jericho_screen_buffer;
}
void jericho_clear_screen(void) {
  jericho_screen_buffer_ptr = jericho_screen_buffer;
}
/////////////////////////////////////////////////////
...
{ 
  /// START ZAI FIX
  jericho_set_main_window_text(c);
  /// END ZAI FIX
