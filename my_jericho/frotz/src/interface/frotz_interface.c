...
static int set_save_name(const char *filename) {
  char *name = realloc(f_setup.save_name, strlen(filename) + 1);
  if (name == NULL) {
    return 0;
  }
  f_setup.save_name = name;
...
char *get_response_from_screen(){
  char* text;
  text = jericho_get_main_window_text();
  strcpy(world, text);
...
char* setup(char *story_file, int seed, void *rom, size_t rom_size) {
  ...
  /// START ZAI FIX
  if(1)
   return get_response_from_screen();
  else
   return world;
  /// END ZAI FIX
}
